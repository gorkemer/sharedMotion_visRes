...
sum_stats <- fgmdata.c[, .(mean_response = mean(responseAR),
                           ...
                           sample_n = .N),
                       by = .(sub, cuedAR, global_org)];
sum_stats_2 <- sum_stats[, .(mean_response = mean(mean_response),
                             sd_response = sd(mean_response),
                             se_response = sd(mean_response) / sqrt(.N),
                             sample_n = .N),
                         by = .(cuedAR, global_org)] 
globalOrg_exp1 <- ggplot(sum_stats, aes(x = cuedAR, y = mean_response, group = as.factor(global_org), color = as.factor(global_org))) +
  ...
  theme(legend.key = element_blank(), strip.background = element_rect(colour="white", fill="white") );globalOrg_exp1
##
