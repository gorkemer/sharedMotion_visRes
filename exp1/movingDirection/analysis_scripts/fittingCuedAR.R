...
plotCurves <- function(){
  ...
  x_values <- seq(-0.47, 0.47, length.out = 21)
  # Create an empty plot
  plot <- ggplot() +
    labs(x = "Cued Aspect Ratio (log)", y = "Cumulative Probability of Responding 'Tall'", title = "Stacked Cumulative Gaussian Curves") +
    theme_minimal()
  test <- jnd_df[jnd_df$id %in% excluded_people,]
  for (i in seq_along(test$obs_MU)) {
    mu <- test$obs_MU[i]
    sigma <- test$obs_SIGMA[i]
    sub_id <- test$id[i]
    cumulative_probs <- pnorm(x_values, mean = mu, sd = sigma)
    # Add the curve to the plot
    plot <- plot + geom_line(data = data.frame(x = x_values, y = cumulative_probs), aes(x = x, y = y), color = "#808080")
  }
  plot;
  #
  test <- jnd_df[!(jnd_df$id %in% excluded_people),]
  for (i in seq_along(test$obs_MU)) {
    ...
    cumulative_probs <- pnorm(x_values, mean = mu, sd = sigma)
    # Add the curve to the plot
    plot <- plot + geom_line(data = data.frame(x = x_values, y = cumulative_probs), aes(x = x, y = y), color = "#F8766D")
  }
  plot;
  return(plot)
}
loadExp1 <- function(){
  ...
  jnd_df$id[jnd_df$jnd >0.5] 
  jnd_df$id[jnd_df$jnd >0.43] 
  mean(jnd_df$jnd)
  sd(jnd_df$jnd)
  hist(jnd_df$jnd)
