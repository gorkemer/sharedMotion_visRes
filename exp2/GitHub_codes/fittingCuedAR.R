...
loadExp2 <- function(){
  ...
  bgmndata.r = subset(bgmndata, bgmndata$randomTrialsR1C0 == 1);   bgmdata.r$responseAR_normed <- bgmndata$normedR_indv[bgmndata$sameDirection1S0D_R2 == 2];#fgmdata <- bgmdata.r ### here random
  fgmdata <- bgmdata.c
  ...
  sample_data <- aggregate(respTall ~ cuedAR + sub, fgmdata[fgmdata$respTall %in% c(0,1) & fgmdata$sub == 19182,], mean) 
  ...
  for (i in unique(fgmdata$sub)){
    ...
    if (!is.null(fit)) {
      ...
      sigma_list <- append(sigma_list, sigma)
      # Print the results
      cat("Estimated mu:", mu, "\n")
      cat("Estimated sigma:", sigma, "\n")
      cat("Just Noticeable Difference (JND):", JND, "\n")
  ...
  jnd_df <- data.frame(id_list[-1], jnd_list[-1], mu_list[-1], sigma_list[-1]); colnames(jnd_df) <- c("id", "jnd", "obs_MU", "obs_SIGMA");
  ...
  mean(jnd_df$jnd)
  sd(jnd_df$jnd)
  excluded_people <- jnd_df$id[jnd_df$jnd >0.6] 
  fgmdata.c <- fgmdata[!(fgmdata$sub %in% excluded_people), ]
  length(unique(fgmdata.c$sub))
  curvedPlot_exp2 <- plotCurves()
  curvedPlot_exp2;
  return(fgmdata.c)
...
sample_data <- aggregate(responseAR_normed ~ uncuedAR * sameDirection1S0D + sub, fgmdata.c[fgmdata.c$randomTrialsR1C0 ==0,], mean)
modelCuedAR <- lmer(responseAR_normed ~ uncuedAR * sameDirection1S0D + (1 | sub), data = sample_data, REML = FALSE)
summary(modelCuedAR)
#
ggplot(jnd_df[!(jnd_df$id %in% excluded_people),], aes(id, jnd, label = id)) + geom_label(alpha = 1/3)
