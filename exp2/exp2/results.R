...
getStats <- function(){
  ...
  sum_stats <- fgmdata[, .(mean_response = mean(responseAR),
                             ...
                             sample_n = .N),
                         by = .(sub, cuedAR, uncuedAR, sameDirection1S0D, global_org1W0B)]; #global_org1W0B #global_org
  #
  ...
  # globalorg:
  globalOrgModel <- update(uncuedAR_model, .~. + global_org1W0B); #lmer(mean_response ~ cuedAR + (uncuedAR * as.factor(sameDirection1S0D)) + as.factor(global_org1W0B)  + (1 | sub), data = sum_stats, REML = FALSE) 
